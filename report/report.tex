...
Ji et~al.~\cite{ji2023survey} provide a comprehensive survey distinguishing extrinsic hallucinations (contradicting external knowledge) from intrinsic hallucinations (self-contradictory). This evaluation focuses on extrinsic hallucinations. Maynez et~al.~\cite{maynez2020faithfulness} study faithfulness in abstractive summarisation, establishing that factual correctness and output formatting are distinct dimensions of generation quality. Building on this insight, the present evaluation separately tracks format violations, hallucinations, and refusals.
...
The dataset intentionally includes specialised, modern tools (Polars, Snowflake) where training data may be limited, enabling evaluation of model behaviour at the frontier of their knowledge.

\subsection{AI Models Tested}

\textbf{Google Gemini~2.5 Flash}: Released July~2025, a lightweight multimodal transformer with a 1-million-token context window, accessed via the Google Gemini API.

\textbf{Cohere Command~R7B}: Released December~2024, a 7-billion-parameter decoder-only model designed for enterprise deployments, accessed via the Cohere~v2 Chat API (model~identifier: \texttt{command-r7b-12-2024}).
...
The large gaps in specialised domains (Snowflake: 60\,pp, Spark: 50\,pp) are statistically robust, while differences in fundamental domains (SQL: 7\,pp) are not significant.
...
\textbf{Domain Specialisation and Training Data.} Polars (released~$\sim$2020) and modern Snowflake features (2023--2024) post-date most LLM training cutoffs. Knowledge questions were intentionally selected from modern, specialised domains where models are expected to have gaps.
...
A single prompt formulation per category is used; response patterns might change with different prompts. All generations used default temperature settings. Only zero-shot performance is evaluated; few-shot examples might improve results. The Piston API sandbox's 5-second timeout may not reflect all production scenarios.

\subsection{External Validity}

Only two models were evaluated. Results do not generalise to GPT-4, Claude~3, Llama~3, or other LLMs. Model size disparity (Cohere~7B vs.\ Gemini's unknown but likely larger architecture) affects comparisons. The knowledge questions focus on modern data tools; generalisation to medical, legal, or scientific domains is unclear. All questions are English-only.

\subsection{Conclusion Validity}

Binary scoring misses partial correctness. Category sample sizes are small ($n=10$--$15$), yielding wide confidence intervals. Bonferroni correction was applied to control false positives across 8~category comparisons. No inter-rater reliability study was conducted; the exact-match criterion is stricter than typical human judgement.
...
Exact training data cutoffs for the models are not publicly available, limiting the ability to distinguish training data gaps from architectural limitations.
...
\begin{enumerate}[leftmargin=*]
  \item Expanded model evaluation across GPT-4, Claude~3, Llama~3, and open-source models.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Specialised tools (Snowflake, Polars) expose significant gaps, with performance degrading to 40\% in the worst case.
\end{enumerate}
